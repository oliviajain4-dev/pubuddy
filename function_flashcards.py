# =========================================================
# 📦 [프로젝트 정보 - 고정 영역]
# ---------------------------------------------------------
# 프로젝트명 : PyHelper (파이썬 지식 도감)
# 개발자     : oliviajain4
# 이메일     : oliviajain4@gmail.com
# 원격 저장소: https://github.com/oliviajain4/pyhelper.git
# =========================================================

# =========================================================
# ⚙️ [1️⃣ Streamlit 설정 - 고정 구간]
# =========================================================
import streamlit as st

st.set_page_config(
    page_title="PyHelper - 파이썬 도감",
    page_icon="🐍",
    layout="centered"
)

if "saved_cards" not in st.session_state:
    st.session_state.saved_cards = []

# =========================================================
# 🧠 [2️⃣ 기본 데이터 구간 - 학습 카드]
# =========================================================
python_knowledge = {
    "print": {
        "type": "기본 함수",
        "importance": 5,
        "desc": "콘솔(터미널)에 데이터를 출력합니다.",
        "example": "print('Hello, world!')",
        "usage": "가장 기본적인 출력 함수입니다. 디버깅(코드 확인)용으로도 자주 사용합니다.",
        "related": ["input", "len", "type"]
    },
    "len": {
        "type": "기본 함수",
        "importance": 5,
        "desc": "객체(문자열, 리스트 등)의 길이를 반환합니다.",
        "example": "len('hello')  # 5",
        "usage": "문자열, 리스트, 딕셔너리의 길이를 구할 때 사용합니다.",
        "related": ["count", "sum"]
    },
    "append": {
        "type": "리스트 메서드",
        "importance": 5,
        "desc": "리스트 끝에 새 요소를 추가합니다.",
        "example": "my_list = [1, 2]\nmy_list.append(3)\nprint(my_list)  # [1, 2, 3]",
        "usage": "리스트에 데이터를 누적하거나, 반복문에서 하나씩 추가할 때 사용합니다.",
        "related": ["list", "extend", "insert"]
    },
    "remove": {
        "type": "리스트 메서드",
        "importance": 4,
        "desc": "리스트에서 첫 번째로 일치하는 요소를 제거합니다.",
        "example": "my_list = [1, 2, 3, 2]\nmy_list.remove(2)\nprint(my_list)  # [1, 3, 2]",
        "usage": "리스트 안에서 특정 값이 존재할 때만 제거됩니다. 값이 없으면 오류 발생!",
        "related": ["pop", "del"]
    },
    "if": {
        "type": "제어문 (Control Flow)",
        "importance": 5,
        "desc": "조건에 따라 코드 실행 흐름을 제어합니다.",
        "example": "if score >= 60:\n    print('합격')\nelse:\n    print('불합격')",
        "usage": "조건 판단, 분기 처리를 할 때 사용됩니다.",
        "related": ["for", "while"]
    }
}

# =========================================================
# 🧩 [3️⃣ 로직 함수 구간]
# =========================================================
def show_card(query: str):
    """입력한 단어의 카드 정보 표시"""
    data = python_knowledge.get(query.lower())
    if not data:
        st.warning("아직 등록되지 않은 개념이에요 😅")
        return

    st.markdown(f"### 📘 {query}()")
    st.markdown(f"**📂 분류:** {data['type']}")
    st.markdown(f"**⭐ 중요도:** {'★' * data['importance']}")
    st.markdown(f"**📖 설명:** {data['desc']}")
    st.code(data['example'], language="python")
    st.markdown(f"**🪄 사용방법:** {data['usage']}")
    st.markdown(f"**🔗 관련 개념:** {', '.join(data['related'])}")

    # 저장 버튼
    if st.button("💾 이 카드 저장하기", key=query):
        if query not in st.session_state.saved_cards:
            st.session_state.saved_cards.append(query)
            st.success(f"'{query}'를 내 보관함에 저장했어요! 🎉")
        else:
            st.info("이미 저장된 카드예요 😊")

# =========================================================
# 🎨 [4️⃣ 출력/UI 구간 - 수정 자유]
# =========================================================
st.title("🐍 PyHelper — 파이썬 지식 도감")
st.caption("기초부터 차근차근, 내가 직접 정리하는 파이썬 학습 챗봇 📘")

query = st.text_input("찾고 싶은 파이썬 함수나 개념을 입력하세요 (예: append, if, len):")

if query:
    show_card(query)

with st.sidebar:
    st.header("📂 내 보관함")
    if st.session_state.saved_cards:
        for saved in st.session_state.saved_cards:
            st.write(f"- {saved}")
    else:
        st.caption("아직 저장된 카드가 없어요 💡")

    st.markdown("---")
    st.info("다음 목표: 🃏 암기 카드(앞/뒤 보기) 모드 추가 예정!")
    
